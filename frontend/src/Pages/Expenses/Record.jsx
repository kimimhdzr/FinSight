...
const Record = () => {
  ...
  const getPieData = (payments, selectedMonth, selectedYear) => {
    ...
    filtered.forEach((p) => {
      totalsByCategory[p.category] =
        (totalsByCategory[p.category] || 0) + parseFloat(p.amount);
    });
  ...
  const handleSavePayment = async () => {
    try {
      ...
      alert(
        "Error saving payment. Is your backend running at http://localhost:5000?"
      );
    }
  };
  ...
  const getExpenseBreakdown = () => {
    ...
    const breakdown = Object.entries(currentTotals).map(
                    ...
                    {months.map((month) => (
                      <div
                        ...
                        style={{
                          ...dropdownItemStyle,
                          backgroundColor:
                            month === selectedMonth ? "#007bff" : "transparent",
                    ...
                    {years.map((year) => (
                      <div
                        ...
                        style={{
                          ...dropdownItemStyle,
                          backgroundColor:
                            year === selectedYear ? "#007bff" : "transparent",
                  ...
                  {breakdownTop2.map((item, i) => (
                    <div key={i} style={expenseItemStyle}>
                      <div>
                        ...
                        >
                          RM {item.currentAmount.toFixed(2)}
                        </div>
                      </div>
                      <span
                  ...
                  <div style={categoryGridStyle}>
                    {expenseCategories.map((category) => (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          setPaymentForm((prev) => ({ ...prev, category }))
                        }
                        style={{
                          ...categoryButtonStyle,
                          backgroundColor:
                            paymentForm.category === category
                              ? "#007bff"
                              : "#f8f9fa",
                          color:
                            paymentForm.category === category
              ...
              <div style={modalActionsStyle}>
                <button
                  ...
                  style={{
                    ...saveButtonStyle,
                    opacity:
                      !paymentForm.amount || !paymentForm.description ? 0.5 : 1,
                    cursor:
                      !paymentForm.amount || !paymentForm.description
                        ? "not-allowed"
                        : "pointer",
                  }}
